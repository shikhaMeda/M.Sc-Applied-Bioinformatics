...
for(i in 1:ncol(devoid_outliers)){
  devoid_outliers[is.na(devoid_outliers[,i]), i] <- Data_floral.origin-mean(devoid_outliers[,i], na.rm = TRUE)
}

ftir_spectra <- devoid_outliers                                     # mean centered spectra obtained with categories
ftir_spectra <- ftir_spectra[,-1867]                               # remove the floral categories

# SNV
tFTIR_spectra <- t(ftir_spectra)                                            # transpose spectra
FTIR_snv <- scale(tFTIR_spectra, center = T, scale = T)  
...
library(magrittr)
ftir_Data$Fl.1 %<>% factor                                                  # specify floral category as factors
ftir_Data$Fl.1 <- as.numeric(ftir_Data$Fl.1)                                # then factors as numric values (4 levels)
...
x11()
barplot(misclassifications_nb_floral, ylab="misclassifications over 100 iterations", ylim = c(0,100), xlab="class", main=paste("naiveBayes",": Misclassified samples per class"), cex.names=0.5)
